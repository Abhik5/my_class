...
int background_ncdm_distribution(
  ...
  int n_ncdm,lastidx;
  double B_sp,E,N0,s,y1,q1;       //for non-thermal distribution
  //double chi;     //for DW
  double ksi;
...
/*For non-thermal distribution*/
	if (n_ncdm==1){
   		if(q>0.05 && q<2.60){ //in the paper, q<4
		//*f0 = (32*(B_sp)*(N0))*(-077.73489905293605*pow(10,-5)+0106.59540957513952*pow(10,-4)*(q*s*0.25)+054.76566626911941*pow(10,-4)*pow((q*s*0.25),2)-367.54684398069876*pow(10,-6)*pow((q*s*0.25),3)+888.2798517168838*pow(10,-8)*pow((q*s*0.25),4)-949.2883268026593*pow(10,-10)*pow((q*s*0.25),5)+380.3996806865481*pow(10,-12)*pow((q*s*0.25),6))/(_PI_*pow(E,3)*pow(q,3)*pow(s,3));
		//);//);//
		y1=-46.0863939863893*pow(10,-4)+1.49998135155539053*pow(10,-2)*(q*s*0.25)+4.989381247260306*pow(10,-3)*pow(q*s*0.25,2)-3.965919521923607*pow(10,-4)*pow(q*s*0.25,3)+1.6661036879609886*pow(10,-5)*pow(q*s*0.25,4)-6.4447104155538676*pow(10,-7)*pow(q*s*0.25,5)+2.0441626364742974*pow(10,-8)*pow(q*s*0.25,6)-4.2465621739426746*pow(10,-10)*pow(q*s*0.25,7)+5.2590421190024246*pow(10,-12)*pow(q*s*0.25,8)-3.541165695653848*pow(10,-14)*pow(q*s*0.25,9)+1.0026773320507325*pow(10,-16)*pow(q*s*0.25,10);
		*f0=(32*(B_sp)*(N0)*y1/(_PI_*pow(E,3)*pow(q,3)*pow(s,3))) ;
	}
	else
	*f0=0;
	}
/*For different temperature thermal distribution*/
	/*if (n_ncdm==1){
		*f0 = 1.0/pow(2*_PI_,3)*(1./(exp(q-ksi)+1.) +1./(exp(q+ksi)+1.));
	}*/
/*For DW*/
	/*if (n_ncdm==1){
		*f0 = chi/pow(2*_PI_,3)*(1./(exp(q-ksi)+1.) +1./(exp(q+ksi)+1.));
	}*/	
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      for (n_ncdm=0;n_ncdm<pba->N_ncdm; n_ncdm++) {
        ...
        /* inform the user also about the value of the ncdm
           masses in eV and about the ratio [m/omega_ncdm] in eV (the usual 93 point something)*/
...
int background_output_titles(
  ...
  //================================================================================================================================
  class_store_columntitle(titles,"Neff",_TRUE_);
  class_store_columntitle(titles,"B_sp",_TRUE_);
  class_store_columntitle(titles,"E",_TRUE_);
