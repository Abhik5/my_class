...
#
var_name = 'w_b'
var_array = np.linspace(10**(-3),0.1,5)
var_num = len(var_array)
var_legend = r'$\omega_b$'
var_figname = 'w_b'
#
...
M = {}
#
for i, omega_b in enumerate(var_array):
    #
    ...
    print (' * Compute with %s=%e, %s=%e, %s=%e'%('omega_b',omega_b,'omega_cdm',omega_cdm,'h',h))
    #
    # call CLASS
    #
    M[i] = Class()
...
for i, omega_b in enumerate(var_array):
    ...
    if i == 0:
        ...
        ax_TT.semilogx(ll[i],clTT[i]/clTT[0],
                       color=var_color,alpha=var_alpha,linestyle='-',
                      label=r'$omega_b=%g$'%(omega_b))
#
# output of P(k) figure
#
ax_Pk.set_xlim([1.e-3,3.])
ax_Pk.set_ylim([0.0,1.0])
ax_Pk.set_xlabel(r'$k \,\,\,\, [h^{-1}\mathrm{Mpc}]$')
ax_Pk.set_ylabel(r'$P(k)/P(k)[omega_b=0.001]$')
...
ax_TT.set_ylabel(r'$C_\ell^\mathrm{TT}/C_\ell^\mathrm{TT}(omega_b=0.001)$')
